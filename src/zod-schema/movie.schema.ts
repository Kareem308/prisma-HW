...
export const moveSchema=z.object({
    body:z.object({
        name:z.string({required_error:'name is required'}).min(2,'Move name min 2 char'),
...
export const getMoveByNameSchema=z.object({
    params:z.object({
        name:z.string({required_error:'name is required'}).min(2,'Move name must be min 2 char'),
    })
})
export const getMoveByGenreScema=z.object({
    params:z.object({
        genre:z.string({required_error:"****"}),
    })
})
